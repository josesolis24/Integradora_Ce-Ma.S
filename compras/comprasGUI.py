...
class ComprasGUI(tk.Toplevel):
    def __init__(self, menu_principal):
        ...
        tk.Label(self, text="Módulo de Compras", font=("Arial", 18, "bold")).pack(pady=15)

        frame = tk.Frame(self)
        frame.pack(pady=15)

        tk.Label(frame, text="ID Producto:", font=("Arial", 12)).grid(row=0, column=0, padx=10, pady=10)
        self.entry_producto = tk.Entry(frame, width=30)
        self.entry_producto.grid(row=0, column=1)

        tk.Label(frame, text="Cantidad:", font=("Arial", 12)).grid(row=1, column=0, padx=10, pady=10)
        self.entry_cantidad = tk.Entry(frame, width=30)
        self.entry_cantidad.grid(row=1, column=1)

        tk.Button(self, text="Registrar Compra", width=30,
                  command=self.registrar_compra).pack(pady=15)

        self.tabla = ttk.Treeview(self, columns=("ID", "Producto", "Cantidad", "Fecha"), show="headings")
        ...
        tk.Button(self, text="Eliminar Compra Seleccionada", width=30,
                  command=self.eliminar_compra).pack(pady=5)

        tk.Button(self, text="⬅ Volver al Menú Principal", width=30,
                  command=self.volver_al_menu).pack(pady=15)
    ...
    def mostrar_compras(self):
        ...
        conexion = crear_conexion()
        cursor = conexion.cursor()
        cursor.execute("SELECT id, id_producto, cantidad, DATE_FORMAT(fecha, '%d-%m-%Y') FROM compras")
